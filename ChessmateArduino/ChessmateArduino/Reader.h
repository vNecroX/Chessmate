...
class Reader
{
  private:
    ...
    String listPut; // Lista de NUEVAS posiciones donde se ha encontrado una pieza.
    String listLeft; // Lista de NUEVAS posiciones en donde no se ha encontrado una pieza.
  ...
  public:
    ...
    void scanBoard()                         
    {
      ...
      getNewOccupiedPositions();
      getNewNoOccupiedPositions();
    ...
    // Getters of positions with pieces, without pieces, with new pieces, with new no pieces
    ...
    boolean anyKeyLeft()
    {
      if((listLeft.length()/2) == 1)
        return true;
      else
        return false;
    }

    boolean moreThanOneKeysPut()
    {
      if((listPut.length()/2) > 1)
         return true;
      else
    ...
    boolean moreThanxKeysIn(int limit)
    {
      if((listIn.length()/2) > limit)
        return true;
      else
        return false;
    }

    boolean moreThanxKeysOut(int limit)
    {
      if((listOut.length()/2) > limit)
        return true;
      else
        return false;
    }

    boolean anyChangeDetected()
    {
        if(moreThanxKeysIn(32) || moreThanxKeysOut(64) || moreThanOneKeysLeft() || moreThanOneKeysPut() || anyKeyLeft() || anyKeyPut())
        {
            return true;
        }
        else
    ...
    void showListIn()
    {
      Serial.println("Occupied positions");
    ...
    void showListOut()
    {
      Serial.println("No occupied positions");
    ...
    void showAuxListIn()
    {
      Serial.println("Last occupied positions");
    ...
    void showAuxListOut()
    {
      Serial.println("Last no occupied positions");
    ...
    void showListPut()
    {
      Serial.println("New occupied positions");
    ...
    void showListLeft()
    {
      Serial.println("New no occupied positions");
    ...
    void getNewNoOccupiedPositions()
    {
      String aux = "";
      String aux2 = "";
